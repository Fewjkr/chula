...
for i in range(start, end):
    ...
    # หัวการ์ด: ยังโชว์แหล่งข้อมูลที่หัวข้อได้ (แต่จะไม่ซ้ำในข้อมูลหลัก)
    title = f"{common} • {cas} • {src}"

    with st.container(border=True):
        st.markdown(f"### {title}")

        # แถวบน: สรุป (ลดความเป็นหัวข้อของ "กรณีที่ใช้" ให้เป็นบรรทัดเรียบ ๆ)
        ...
        with b:
            ...
            st.write(f"กรณีที่ใช้: {usecase}")  # <-- ไม่ใช้ st.caption('กรณีที่ใช้') แล้ว
        with c:
        ...
        # แถวล่าง: รายละเอียด (เอา "แหล่งข้อมูล: ..." ออกตามที่วงไว้)
